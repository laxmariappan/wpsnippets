...
export function getPostSlugs(folder) {
  const contentDirectory = resolve(process.cwd(), ParentDirectory);
  return readdirSync(contentDirectory+ '/' + folder)
}

export function getPostBySlug(slug, fields = [], folder) {
  const realSlug = slug.replace(/\.md$/, '')
  const postsDirectory = resolve(process.cwd(), ParentDirectory + '/' + folder)
